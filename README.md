...
管理者アカウントでログインする場合は、以下の認証情報を使用してください：

- **メールアドレス**: `admin@example.com`
- **パスワード**: `admin123`

管理画面: http://localhost:3000/admin（ローカル環境）

---

## 📖 使い方ガイド（初めての方へ）

CheeRainは簡単3ステップで応援NFTを発行できます！
...
3. メールアドレスとパスワードを入力
4. アカウント種別を選択:
   - **一般ユーザー**: 選手への応援メッセージ送信、NFT発行
   - **管理者**: システム管理、統計閲覧、ユーザー管理

### ステップ2: ブロックチェーンNFTを発行（任意）

![NFT一覧](docs/images/nft-list.png)

1. マイページの「ブロックチェーンNFT」タブを選択
2. 「NFT発行」ボタンをクリック
3. 応援したい選手を選択
4. 応援メッセージを入力
5. 支払い金額を選択（500円、1000円など）
6. MetaMaskでウォレット接続してNFT発行

発行したNFTは永久にブロックチェーン上に記録され、あなただけのコレクションになります！

### ステップ3: 会場来場でポイントゲット

マイページから会場コードを入力すると、スタジアム来場特典としてポイントが獲得できます。
...
2. **Admin権限の付与**（管理画面にアクセスしたい場合）
   - phpMyAdmin (http://localhost:8081) にアクセス
   - `users` テーブルを開く
   - 作成したユーザーの `role` を `ADMIN` に変更
   - 管理画面にアクセス: http://localhost:3000/admin
...
**全テストが正常に完了しています**（2025年10月時点）：

#### 1. E2Eテスト（Playwright）
- ✅ **69 passed / 16 skipped / 0 failed**
- **対象ブラウザ**: Chrome, Firefox, Safari, Mobile Chrome, Mobile Safari
- **テスト内容**:
  - 認証フロー（ログイン・サインアップ・バリデーション）
  - ナビゲーション（ページ遷移・戻るボタン）
  - トップページ基本機能
  - モバイルメニュー動作確認

#### 2. フロントエンドユニットテスト（Jest + React Testing Library）
- ✅ **全テスト成功**
- **テスト対象**:
  - `contentModeration.ts`: 不適切なコンテンツ検出ロジック
    - NGワード検出、誹謗中傷判定、スパム検出
    - 安全なコンテンツの正常判定
  - Reactコンポーネントのレンダリング
  - ユーザーインタラクション

#### 3. バックエンド統合テスト（Spring Boot + JUnit 5）
- ✅ **全テスト成功**
- **テスト対象**:
  - **AuthController**: ユーザー登録・ログイン・JWT認証
  - **NFTController**: NFT発行・一覧取得・削除
  - **PlayerController**: 選手データ取得
  - **MatchController**: 試合データ取得
  - **VenueCodeController**: 会場コード発行・検証
- **テスト手法**:
...
![NFT一覧画面](docs/images/nft-list.png)
...
- 支払額（500円、1000円など）
- 発行日時
- PolygonscanでNFTを確認できるリンク

### 2. マイページ

マイページでは自分の応援履歴とNFTコレクションを確認できます：
